...
fn main() {
    ...
    let db = cli
        .db
        .as_deref()
        .unwrap_or("postgres://postgres@localhost:8000");
    ...
    match &cli.command {
        ...
        Some(Commands::Add { name, email }) => {
            create_user(&mut conn, name, email).unwrap();
        }
        Some(Commands::List) => {
            for (name, email) in list_user(&mut conn).unwrap() {
                println!("The name {name:#?} and email {email:#?}");
            }
        }
        _ =>
            /*println!("try using oe of the commands")*/
            {}
    }
}
